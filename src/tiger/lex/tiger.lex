...
[[:alpha:]_][[:alnum:]_]* {adjust(); return Parser::ID;}
...
<COMMENT> {
  ...
  .|\n {adjust();}
  <<EOF>> {errormsg_->Error(errormsg_->tok_pos_, "unterminated comment");}
}
...
<STR> {
  ...
  "\\\"" {adjustStr(); string_buf_ += "\"";}
  "\\\\" {adjustStr(); string_buf_ += "\\";}
  "\\"[[:digit:]]{3} {adjustStr(); string_buf_ += (char)(atoi(matched().substr(1).c_str()));}
  "\\"[[:space:]]+"\\" {adjustStr();}
